...
class Solution:
    def minCost(self, grid: List[List[int]], k: int) -> int:
        ...
        # DP table: dp[t][pos] = min cost to reach pos with t teleports
        dp = [[INF] * (m * n) for _ in range(k + 1)]
        dp[0][0] = 0
        
        # Base case: 0 teleports (only normal moves)
        for i in range(m):
            for j in range(n):
                pos = i * n + j
                if i > 0:
        ...
        for t in range(1, k + 1):
            # Build suffix minimum over grid values
            ...
            for i in range(m):
                for j in range(n):
                    ...
                    # Option 1: Don't use teleport, Option 2: Teleport here
                    dp[t][pos] = min(dp[t-1][pos], suffix_min[val])
                    
                    # Option 3: Normal moves from top/left
                    if i > 0:
